...
!
!  THIS MODEL WAS DEVELOPED BY SANDIA NATIONAL LABORATORIES UNDER
!  FWP 0709 FOR THE DEPARTMENT OF ENERGY'S PITTSBURGH ENERGY
!  TECHNOLOGY CENTER AND THE DOE DIVISION OF ENGINEERING AND GEOSCIENCES
!  THROUGH THE OFFICE OF BASIC ENERGY SCIENCES;
!  AND BY THE UNIVERSITY OF UTAH THROUGH FUNDING FROM
!  THE ADVANCED COMBUSTION ENGINEERING RESEARCH CENTER (ACERC), WHICH
!  IS PRINCIPALLY SPONSORED BY THE NATIONAL SCIENCE FOUNDATION, THE
!  STATE OF UTAH, AND BY A CONSORTIUM OF INDUSTRIAL COMPANIES.
!  THE CODE WILL NOT BE FORMALLY LICENSED.  NEITHER THE U.S. OR THE
!  DOE, NOR ANY OF THEIR EMPLOYEES, MAKES ANY WARRANTY, EXPRESS OR IMPLIED,
...
!
!
!  The CPD model is intended to solve pyrolysis rate equations
!  based on a percolative bond-breaking scheme.  This version includes the
!  flash distillation program to distinguish between tar and metaplast.
!  This program also includes a crosslinking scheme.  (January, 1991)
!
!
!  Most recent modifications to this model include (a) the nitrogen release
!  model of Perry, (b) the model of Genetti to break the light gas into
!  species based on a correlation, and (c) slight modification to mdel
!  to account for the mass associated with c0.
!  These modifications were made at BYU by Dominic Genetti in his
!  M.S. Thesis work (1999) and Steve Perry in his Ph.D. work (1999).
!
!
      program cpd
...
!  Output parameters for light gas species distribution in perkot4
      ...
      real*4 mwchar,machar,mwcharold
      integer*2 lib
!  intar = .true. calculates tar molecular weight distribution
!                 in subroutine perkp.
...
!  idiff = .true. for calculations on a differential basis rather than
!           printing out integrated values.  This option worked on early
!           versions, but has not been tested on the latest versions.
      ...
      data y/4*0./,yp/4*0./,ypp/4*0./ypred/4*0./,tim/50*0./, &
          tem/50*0./,ft/35*0./,mt/35*0./,u0/0./              &
          ,kkk/1/,umax/0./,yield/0./ftarold/0.0/
!  y1 = l       labile bridges
!  y2 = del     ends
!  y3 = c       char links
...
!   read in input data for kinetics and output file names
      ...
      read 5,perkot4
5     format(a)
      open(unit=1,file=perkin,status='old')
      open(unit=2,file=perkot,status='unknown')
      open(unit=20,file=perkot2, status='unknown')
      open(unit=21,file=perkot3, status='unknown')
      open(unit=22,file=perkot4, status='unknown')
...
!  read in real mdel from NMR data
      read(1,*)mdel
!  read in mass fractions of C,H,N, and O in parent coal
      read(1,*)fcar
      read(1,*)fhyd
      read(1,*)fnit
      read(1,*)foxy
!  kinetic parameters
      ...
      read(1,*)acr
      read(1,*)ecr
!  kinetic parameters for light gas nitrogen release from char:
!  A. release by reaction with free radicals (fast)
      read(1,*)arad
      read(1,*)erad
      read(1,*)fstable
!  B. release by high T decomposition (slow)
      read(1,*)an
      read(1,*)en0
      read(1,*)ensig
!  pressure in atmospheres
      read(1,*)press
!  save initial char NMR parameters as coal NMR parameters
!  (char parameters calculated independent of those using
!  empirical correlation for mdel)
      mwchar = mw1
      mwcharold = mwchar
      machar = mwchar-sigp1*mdel
      fnt = fnit
!  adjust mdel to correct for c0 (Steve Perry, May 99)
      mdel = mdel/(1.0-c0)
!  empirical correlation to allow a small portion of alpha-carbon to stay with
!  the aromatic cluster
      mdel = mdel-7
...
!   calculate O/C and H/C ratios for light gas model
      xoc = (foxy/16)/(fcar/12)
      yhc = fhyd/(fcar/12)
!   read in input data for temperature profile)
      ma = mw1-sigp1*mdel
...
203   format('c time(ms)      l         c      del/2    g1/2', &
    ...
    '     p')
      write(21,204)
204   format('c time(ms)   temp(K)    MW        Nsite    Nchar', &
      '    fnchar     fntar     fnhcn     fntot')
      write(22,205)
205   format('c time(ms)      fgas     fh20     fco2     fch4', &
      '     fco     fother    yh20     yco2     ych4',          &
      '     yco     yother    Xgas')
      iii = 0
      fgas0 = 0.
      fcross = 0.0
...
! --    DEVOLATILIZATION RATES
        call perks(y,ypp,Tp)
!  free radical light gas nitrogen release mechanism
      if((mw1-mwchar)/mw1.GT.fstable)ypp(4)=ypp(4)-y(4)*arad*&
      exp(-erad/rg/Tp)*(mwcharold-mwchar)/mwchar*&
      machar/mwchar/dt
...
!           print*,'<calchi> calling flash'
          if(fgas.ge.1.e-5)then
           ...
           call flash(fgas,gasmw,ft,mt,fracr,ftar,fmet,&
                     tp,press,nmax,imolw,sumy)
          elseif(fgas.lt.1.e-5)then
           fmet = ftart
           ftar = 0.
          endif
!        print*,'<calchi exiting flash'
          intar = .false.
          fvol = fgas+ftar
          fchar = 1.-fvol
      if(iii.eq.1)then
...
!
!  calculate tar nitrogen yield
!  (assumes tar is released before light gas and HCN)
...
!  fraction of original nitrogen released as tar
      fntar = yntar/fnit
!  fraction of original nitrogen released as light gas (diff.)
      fnhcn = 1.0 - fnchar - fntar
!  total fractional release of nitrogen
      fntot = (fnit - fnt*fchar)/fnit


!  DISTRIBUTE LIGHT GAS INTO H20, CO2, CO, CH4, & other HC's

!  yf is a CPD indicator of the fraction of total light gas
...
!  calculate fraction of total mass release that is h2o, co2, ch4,
...
!
      if(mod(iii-1,iprint).eq.0)then
        ...
        write(21,221)tms,tp,mwchar,y(4),fnt,fnchar,fntar,fnhcn,fntot
        write(22,222)tms,fgas,ffgas(1),ffgas(2),ffgas(3),ffgas(4),&
                     ffgas(5),yygas(1),yygas(2),yygas(3),yygas(4),&
                     yygas(5),yf
      endif
...
!  calculate rate of formation of char
      yp(3) = kb*l/(rho+1.)
!  calculate rate of high T (slow) nitrogen loss (as HCN)
      yp(4) = -kn*fnca
      return
      end
! ------------------------------------------------------------------
...
!
      ...
      c = y(3)
      p = l+c
!      print*,' <perk>l,del,c,g1,g2,g,p,mgas,mtot=',
!     x               l,del,c,g1,g2,g,p,mgas,mtot
      if(intar)then
        if(p.gt.0.9999)then
          delfac = 1.
        else
...
!  calculate tar molecular weight distribution
      ...
      iprint = iprint+1
!      if(iprint.eq.1)
!     x  open(unit=25,name='perktar.out',type='unknown')
      ftsum = 0.0
      do 20 n=1,nmax
         tn = n*(sig-1.)+2
         xm = n*sig+1.
...
10    continue
      ...
      xmw(1) = gasmw
      Ftot = Ftot + f(1)
!  get mole fraction of components in the feed stream
!  and compute equilibrium contants k(i) from vapor pressure and
!  Raoults law expression
      sum = 0.0
      do 30 ii=1,ntot
        sum = sum + f(ii)
...
81      continue
        stop
100     continue
!  now calculate molecular weight distributions on a
!  light-gas free basis, wt fractions
        Vtot = Ftot*x2
        Ltot = Ftot-Vtot
        VoL = Vtot/Ltot
        sumx = 0.0
...
!  This program calculates the distribution of light gas species
!  based on a look up table of reference data
...
!  xz = The index used in correlation.  In main program it
...
!  *****************************************************************
        if(inside)then
!  This interpolation scheme is taken from Zhao et al., 25th Symp.
!  on Comb. (Int.), 1994/pp. 553-560.
...
!  calculate S and R, the weighted fraction of two of the points
!  the weighted fraction of other point will be 1-R-S
...
!  do loop to calculate the light gas composition of each reference
...
!       ************************************************************
        if(inside)then
!  yygas(k) = the fraction of light gas that is k
!  1=h20, 2=co2, 3=ch4, 4=co
...
!       print*,'R and S = ',R,S
!       print*,'fh2o, fco2, fch4, fco, & fother =',yygas(1),
!    x             yygas(2),yygas(3),yygas(4),yygas(5)
